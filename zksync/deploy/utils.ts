...
export const deployFactoryContract = async (
  contractArtifactName: string,
  constructorArguments?: any[],
  depsContractNames?: string[],
  options?: DeployContractOptions
) => {
  const wallet = options?.wallet ?? getWallet();
  const deployer = new Deployer(hre, wallet);

  const factoryArtifact = await deployer.loadArtifact(contractArtifactName);
  const factoryDepsArtifacts = await Promise.all(
    depsContractNames?.map((contractName) =>
      deployer.loadArtifact(contractName)
    ) ?? []
  );
  ...
  const factory = await deployer.deploy(
    ...
    undefined,
    undefined,
    factoryDepsArtifacts.map((artifact) => artifact.bytecode)
  );
  ...
  console.log(`\n"${factoryArtifact.contractName}" was successfully deployed:`);
